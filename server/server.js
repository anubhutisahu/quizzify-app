...
app.get('/', (_req, res) => {
  ...
  const techRows = techStack.map(function(item) {
    return '<div style="display:flex;justify-content:space-between;align-items:center;padding:8px 12px;background:var(--bg3);border-radius:8px;font-size:0.83rem">'
      + '<span style="color:var(--text2)">' + item[0] + '</span>'
      + '<span style="color:var(--accent2);font-weight:600">' + item[1] + '</span>'
      + '</div>';
  }).join('');
  ...
  const html = '<!DOCTYPE html>'
    ...
    + '<style>'
    + ':root{--bg:#0a0a0f;--bg2:#111118;--bg3:#1a1a26;--surface:#1e1e2e;'
    + '--border:rgba(255,255,255,0.08);--border2:rgba(255,255,255,0.15);'
    + '--accent:#7c6fcd;--accent2:#a78bfa;--gold:#f59e0b;--gold2:#fcd34d;'
    + '--green:#34d399;--red:#f87171;--blue:#60a5fa;--text:#f0f0f8;--text2:#a0a0b8;}'
    ...
    + '.section{background:var(--surface);border:1px solid var(--border);border-radius:14px;padding:20px;margin-bottom:20px;position:relative;overflow:hidden}'
    + '.section::before{content:"";position:absolute;top:0;left:0;right:0;height:2px;background:linear-gradient(90deg,var(--gold),var(--gold2));opacity:0.6}'
    + '.section-title{font-family:"Syne",sans-serif;font-size:0.8rem;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:var(--text2);margin-bottom:16px}'
    ...
    + '.room-id{font-family:"Syne",sans-serif;font-size:1.4rem;font-weight:800;color:var(--gold2);letter-spacing:4px;margin-bottom:8px}'
    + '.room-status{display:inline-flex;align-items:center;gap:6px;padding:3px 10px;border-radius:20px;font-size:0.75rem;font-weight:600;margin-bottom:10px}'
    ...
    + '.player-chip{display:inline-flex;align-items:center;gap:5px;background:rgba(255,255,255,0.05);border:1px solid var(--border);border-radius:20px;padding:3px 10px;font-size:0.78rem;margin:3px}'
    ...
    + '.event-item{display:flex;gap:12px;padding:8px 0;border-bottom:1px solid var(--border);font-size:0.83rem;align-items:flex-start}'
    + '.event-item:last-child{border-bottom:none}'
    + '.event-time{color:var(--accent2);font-weight:600;flex-shrink:0;font-size:0.78rem;font-family:monospace;padding-top:1px}'
    + '.event-msg{color:var(--text2)}'
    + '.info-bar{display:flex;align-items:center;justify-content:space-between;background:var(--bg3);border:1px solid var(--border);border-radius:10px;padding:12px 18px;margin-bottom:24px;flex-wrap:wrap;gap:10px}'
    + '.info-item{font-size:0.82rem;color:var(--text2)}'
    + '.info-item strong{color:var(--text)}'
    + '.refresh-btn{background:rgba(124,111,205,0.15);border:1px solid var(--accent);border-radius:8px;padding:6px 14px;color:var(--accent2);font-size:0.82rem;cursor:pointer;font-family:"DM Sans",sans-serif}'
    + '.two-col{display:grid;grid-template-columns:1fr 1fr;gap:16px}'
    + '@media(max-width:700px){.two-col{grid-template-columns:1fr}}'
    + '</style></head><body>'
    + '<h1>Quizzify Dashboard</h1>'
    + '<p class="subtitle"><span class="status-dot"></span>Server is live &middot; Auto-refreshes every 5 seconds</p>'
    + '<div class="info-bar">'
    ...
    + '<div class="info-item" id="last-updated">Last updated: &mdash;</div>'
    + '<button class="refresh-btn" onclick="fetchData()">Refresh</button>'
    + '</div>'
    + '<div class="stats-grid">'
    + '<div class="stat-card"><div class="stat-label">Active Rooms</div><div class="stat-value" id="s-rooms">-</div><div class="stat-sub">Currently open</div></div>'
    + '<div class="stat-card"><div class="stat-label">Live Connections</div><div class="stat-value" id="s-conn">-</div><div class="stat-sub">Socket.IO clients</div></div>'
    + '<div class="stat-card"><div class="stat-label">Total Rooms</div><div class="stat-value" id="s-total-rooms">-</div><div class="stat-sub">Since server start</div></div>'
    + '<div class="stat-card"><div class="stat-label">Total Players</div><div class="stat-value" id="s-total-players">-</div><div class="stat-sub">Since server start</div></div>'
    + '<div class="stat-card"><div class="stat-label">Quizzes Run</div><div class="stat-value" id="s-quizzes">-</div><div class="stat-sub">Since server start</div></div>'
    + '<div class="stat-card"><div class="stat-label">Uptime</div><div class="stat-value" id="s-uptime">-</div><div class="stat-sub">Seconds running</div></div>'
    + '</div>'
    + '<div class="section"><div class="section-title">Active Rooms</div><div class="room-grid" id="rooms-container"><div class="empty-state">No active rooms yet</div></div></div>'
    + '<div class="two-col">'
    + '<div class="section"><div class="section-title">Live Event Log</div><div class="event-log" id="event-log"><div class="empty-state">No events yet</div></div></div>'
    + '<div class="section"><div class="section-title">Tech Stack</div><div style="display:flex;flex-direction:column;gap:10px">' + techRows + '</div></div>'
    + '</div>'
    + '<script>'
    ...
    + 'document.getElementById("s-total-rooms").textContent=d.totalRooms;'
    + 'document.getElementById("s-total-players").textContent=d.totalPlayers;'
    + 'document.getElementById("s-quizzes").textContent=d.totalQuizzes;'
    + 'document.getElementById("s-uptime").textContent=d.uptime+"s";'
    + 'document.getElementById("last-updated").textContent="Last updated: "+new Date().toLocaleTimeString();'
    + 'var rc=document.getElementById("rooms-container");'
    + 'if(!d.rooms||d.rooms.length===0){rc.innerHTML="<div class=\"empty-state\">No active rooms right now</div>";}else{'
    + 'rc.innerHTML=d.rooms.map(function(r){'
    + 'var players=r.players.map(function(p){return "<span class=\"player-chip\">"+p.name+"<span class=\"player-score\">"+p.score+"pts</span></span>";}).join("");'
    + 'return "<div class=\"room-card\"><div class=\"room-id\">"+r.roomId+"</div>"'
    + '+"<div class=\"room-status "+r.status+"\">"+r.status.toUpperCase()+"</div>"'
    + '+"<div style=\"font-size:0.78rem;color:var(--text2);margin-bottom:8px\">"+r.questions+" questions &middot; "+r.playerCount+" players</div>"'
    + '+"<div>"+players+"</div></div>";'
    + '}).join("");}'
    + 'var el=document.getElementById("event-log");'
    + 'if(!d.recentEvents||d.recentEvents.length===0){el.innerHTML="<div class=\"empty-state\">No events yet</div>";}else{'
    + 'el.innerHTML=d.recentEvents.map(function(e){return "<div class=\"event-item\"><span class=\"event-time\">"+e.time+"</span><span class=\"event-msg\">"+e.msg+"</span></div>";}).join("");}'
